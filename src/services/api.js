...
export const api = {
  ...
  updateNews: (id, updatedArticle) => axios.put(`${API_BASE_URL}/news/${id}`, updatedArticle),
  ...
  fallbackSentimentAnalysis: (headline, description = '') => {
    const text = `${headline} ${description}`.toLowerCase();
    const positiveWords = ['growth', 'profit', 'gain', 'rise', 'increase', 'strong', 'better', 'good', 'positive', 'up', 'higher', 'surge', 'boost', 'success', 'win', 'beat', 'exceed', 'bullish'];
    const negativeWords = ['loss', 'decline', 'fall', 'drop', 'decrease', 'weak', 'poor', 'negative', 'down', 'lower', 'crash', 'plunge', 'fail', 'miss', 'bearish', 'concern', 'worry', 'risk'];
    ...
    return { sentiment, confidence, reasoning: `Keyword analysis: ${positiveCount} positive, ${negativeCount} negative words found` };
  }
};
